...
async function removeKeys(keys:string[]){await Promise.all(keys.map(key=>AsyncStorage.removeItem(key)));}
async function cleanLegacy(){if(legacyCleared)return;legacyCleared=true;await removeKeys(LEGACY);}
...
export async function loadEstimateReviewSession():Promise<MobileEstimateReviewSession|null>{
  ...
  try{const parsed=JSON.parse(raw) as MobileEstimateReviewSession;if(parsed?.schemaVersion!==1||!parsed.boq?.id||!Array.isArray(parsed.boq.sections))return null;return parsed;}catch{return null;}
}

export async function clearEstimateReviewSession(){await removeKeys([KEY,...LEGACY]);}
