...
const aliases:Record<BoqColumnKey,string[]>={
  ...
  amount:["amount","total","total amount","extended amount","extension","line amount","line total","cost","value","lab amount","labour amount","labor amount","labour cost","labor cost"],
};
...
function matchBoqColumnHeader(value:unknown):BoqColumnKey|null{
  ...
  if(/^(amount|total amount|line total|line amount|extended amount|extension|value|lab amount|labour amount|labor amount|labour cost|labor cost)$/.test(header))return "amount";
  return null;
}
...
function scoreHeader(columns:BoqColumnMap){
  const keys=Object.keys(columns) as BoqColumnKey[];let score=keys.length;
  if(columns.quantity!==undefined)score+=2;if(columns.unit!==undefined)score+=1;if(columns.rate!==undefined)score+=1;if(columns.amount!==undefined)score+=1;if(columns.section!==undefined)score+=1;if(columns.specification!==undefined)score+=1;
  return score;
}
function detectBoqHeaderRow(rows:unknown[][],maxScanRows=60):DetectedBoqHeader|null{
  // Use the earliest strong BOQ header. Later repeated headers must not cause the
  // parser to drop earlier bill sections (a real failure seen in the 2-bedroom BOQ).
  let fallback:DetectedBoqHeader|null=null;
  for(let rowIndex=0;rowIndex<Math.min(rows.length,maxScanRows);rowIndex++){
    const columns=mapBoqHeaderRow(rows[rowIndex]??[]);if(columns.description===undefined)continue;
    const score=scoreHeader(columns);const strong=columns.quantity!==undefined&&(columns.unit!==undefined||columns.rate!==undefined||columns.amount!==undefined);
    if(strong&&score>=7)return{rowIndex,columns,score};
    const hasBoqShape=columns.quantity!==undefined||columns.unit!==undefined||columns.rate!==undefined||columns.amount!==undefined;
    if(hasBoqShape&&(!fallback||score>fallback.score))fallback={rowIndex,columns,score};
  }
  return fallback;
}
...
const carriedSummaryPattern=/\b(carried\s+to\s+summary|to\s+summary|bill\s*(nr\.?|no\.?|number)?\s*\d*\s*total|grand\s+total)\b/i;
...
function sheetUsesItemNumbers(sheet:ParsedWorkbookSheet,header:DetectedBoqHeader){const serialIndex=header.columns.serial;if(serialIndex===undefined)return false;let seen=0;for(let r=header.rowIndex+1;r<Math.min(sheet.rows.length,header.rowIndex+120);r++){const row=sheet.rows[r]??[];if(isRepeatedHeader(row))continue;const serial=text(cell(row,serialIndex));const desc=text(cell(row,header.columns.description));if(serial&&desc){seen++;if(seen>=2)return true;}}return false;}
...
function parseDetailSheet(sheet:ParsedWorkbookSheet,header:DetectedBoqHeader,warnings:BoqImportWarning[]):LocalBoqSection[]|null{
  const sections:LocalBoqSection[]=[];const before=precedingSectionBundle(sheet.rows,header.rowIndex,header.columns);const initialTitle=before.title??(sheet.name||"General");let current=makeSection(sheet.name,initialTitle,0,before.context);let explicitSectionSeen=initialTitle!==(sheet.name||"General");let itemIndex=0;let summaryTail=false;const usesItemNumbers=sheetUsesItemNumbers(sheet,header);
  for(let r=header.rowIndex+1;r<sheet.rows.length;r++){
    ...
    const baseDescription=text(cell(row,header.columns.description));const serial=text(cell(row,header.columns.serial));const unitRaw=text(cell(row,header.columns.unit));const qtyRaw=asNumber(cell(row,header.columns.quantity));const rate=asNumber(cell(row,header.columns.rate));const amountRaw=asNumber(cell(row,header.columns.amount));
    if(!serial&&/^(summary|general summary)$/i.test(baseDescription)&&qtyRaw===null&&rate===null&&amountRaw===null){summaryTail=true;warnings.push({sheet:sheet.name,row:r+1,message:"Summary tail detected and excluded from detailed BOQ items to prevent double counting."});continue;}
    if(summaryTail)continue;
    ...
    const specification=text(cell(row,header.columns.specification));const description=baseDescription&&specification&&normalized(specification)!==normalized(baseDescription)?`${baseDescription} — ${specification}`:baseDescription;const rowSection=text(cell(row,header.columns.section));const combined=description||values.join(" ");const hasMeasuredIdentity=Boolean(baseDescription)&&(qtyRaw!==null||Boolean(unitRaw)||rate!==null||Boolean(serial));
    ...
    const looksLumpSum=/^(ls|l\/s|lump\s*sum|sum|item|lot)$/i.test(unitRaw);
    if(usesItemNumbers&&!serial&&qtyRaw!==null&&rate===null&&amountRaw===null&&!looksLumpSum){
      // In numbered BOQs, calculation/material take-off rows usually sit directly
      // below the priced item without an item number or commercial amount. Keep them
      // out of the BOQ so materials are generated once from the measured work item.
      continue;
    }
    const hasCommercialOrMeasuredData=Boolean(serial)||rate!==null||amountRaw!==null||(!usesItemNumbers&&(qtyRaw!==null||Boolean(unitRaw)))||looksLumpSum;
    ...
    if(quantity===null){if(looksLumpSum||amountRaw!==null||rate!==null||serial){quantity=1;warnings.push({sheet:sheet.name,row:r+1,message:`No numeric quantity was found for “${description.slice(0,70)}”; quantity 1 is shown for review.`});}else{quantity=0;warnings.push({sheet:sheet.name,row:r+1,message:`No numeric quantity was found for “${description.slice(0,70)}”; quantity needs review.`});}}
    ...
    itemIndex++;current.items.push({id:`item-${safeId(sheet.name)}-${r+1}-${itemIndex}`,itemNo:serial||undefined,description,unit,quantity,rate,amount,context:[...current.context,current.title],materialBreakdown:{status:"needs_review",materials:[],assumptions:["Material recipe has not yet been confirmed for this imported BOQ item."]}});
  }
...
function directSupply(value:string){return includesAny(value,["door","window","glazing","glass","sanitary ware","sanitaryware","water closet"," wc ","wash hand basin","whb","shower mixer","mixer tap","sink","extractor fan","water heater","socket","switch","light fitting","light fixture","luminaire","air conditioner","ac unit","wardrobe","kitchen cabinet","cabinet","ironmongery","ridge capping","fascia","facial","wall plate","rafter","purlin","noggin"]);}
function suggestRecipe(value:string):{family:BoqRecipeFamily;label:string;code:string|null;strong:boolean}{
  ...
  if(includesAny(value,["reinforcement","reinforcing bar","rebar","high yield steel","mild steel bar","y8","y10","y12","y16","y20","y25","diameter in columns","diameter in beam","diameter in lintel"]))return{family:"reinforcement",label:"Reinforcement recipe",code:"03",strong:true};
  if(includesAny(value,["formwork","shuttering","mould to concrete","sawn formworks","sides of lintel","sides of roof beam","sides of column","sides of colum"]))return{family:"formwork",label:"Formwork recipe",code:"03",strong:true};
  if(includesAny(value,["reinforced concrete","mass concrete","plain concrete","concrete in","concrete work","concrete slab","concrete beam","concrete column","concrete foundation","oversite concrete","weak concrete infill","lintel (","roof beam (","column base"]))return{family:"concrete",label:"Concrete recipe",code:"03",strong:true};
  if(includesAny(value,["plastering","plaster to","cement sand plaster","rendering","render to","blockwall area","other areas (gable"]))return{family:"plastering",label:"Plastering recipe",code:"09",strong:true};
  if(includesAny(value,["screeding","floor screed","cement screed","screed to","levelling screed","leveling screed","cement wall screeding"]))return{family:"screeding",label:"Screeding recipe",code:"09",strong:true};
  if(includesAny(value,["floor tile","floor tiling","floor - tilling","stone tiling","marble tiling","porcelain floor","ceramic floor","granite floor","marble floor"]))return{family:"floor_tiling",label:"Floor tiling recipe",code:"10",strong:true};
  if(includesAny(value,["wall tile","wall tiling","wall finishing (wall tiling)","ceramic wall","porcelain wall","wall cladding"]))return{family:"wall_tiling",label:"Wall finish recipe",code:"11",strong:true};
  if(includesAny(value,["painting","paint to","paint area","emulsion paint","gloss paint","textured paint","primer coat","internal walls","external wall","gable end"]))return{family:"painting",label:"Painting recipe",code:"13",strong:true};
  if(includesAny(value,["roofing sheet","roof covering","longspan","stone coated","aluminium roofing","aluminum roof covering","roof tile","roof membrane","ridge capping"]))return{family:"roofing",label:"Roofing recipe",code:"06",strong:true};
  if(includesAny(value,["ceiling","gypsum board","plasterboard","pop ceiling","suspended ceiling","external ceiling"]))return{family:"ceiling",label:"Ceiling recipe",code:"12",strong:true};
  if(includesAny(value,["pipework","water supply pipe","drainage pipe","soil pipe","waste pipe","pvc pipe","ppr pipe","plumbing installation","plumbing works"]))return{family:"plumbing_installation",label:"Plumbing installation recipe",code:"15",strong:true};
  if(includesAny(value,["cable","conduit","trunking","wiring","wiiring","electrical installation","distribution board","earthing","electrical fittings"]))return{family:"electrical_installation",label:"Electrical installation recipe",code:"16",strong:true};
  if(directSupply(` ${value} `)){let code:string|null=null;if(includesAny(value,["door","ironmongery"]))code="07";else if(includesAny(value,["window","glazing","glass"]))code="08";else if(includesAny(value,["wardrobe","kitchen cabinet","cabinet"]))code="14";else if(includesAny(value,["sanitary","water closet"," wc ","wash hand basin","whb","shower","sink","mixer","water heater"]))code="15";else if(includesAny(value,["socket","switch","light fitting","light fixture","luminaire","extractor fan"]))code="16";else if(includesAny(value,["air conditioner","ac unit"]))code="17";else if(includesAny(value,["ridge capping","fascia","facial","wall plate","rafter","purlin","noggin"]))code="06";return{family:"direct_supply",label:"Direct supply item",code,strong:true};}
  if(includesAny(value,["excavation","earthwork","earth work","backfilling","back filling","cart away","disposal of excavated","laterite filling","hardcore filling","sharpsand filling","dpm","wire mesh laying"]))return{family:"not_applicable",label:"No material recipe required",code:"02",strong:true};
  ...
  if(includesAny(value,["structural steel","steel frame","steel column","steel beam","steel truss","protective screen","pergolia","pergola"]))return{family:"needs_review",label:"Structural steel recipe needs review",code:"05",strong:true};
