...
export default function Home(){
  ...
  const load=useCallback(async()=>{
    ...
    const [{data:projectRows},{data:commercial},{data:acct},{data:appr},intakeCount,statementCount,documentCount,{data:tx}]=await Promise.all([
      ...
      supabase.from("intake_items").select("id",{count:"exact",head:true}).eq("company_id",w.membership.company_id).in("status",["needs_review","failed"]),
      supabase.from("statement_imports").select("id",{count:"exact",head:true}).eq("company_id",w.membership.company_id).gt("rows_pending_review",0),
      supabase.from("project_document_intelligence").select("id",{count:"exact",head:true}).eq("company_id",w.membership.company_id).eq("review_status","pending"),
    ...
    const cmap=new Map((commercial??[]).map((c:any)=>[c.project_id,c]));setProjects((projectRows??[]).map((p:any)=>({...p,summary:Array.isArray(p.summary)?p.summary[0]:p.summary,commercial:cmap.get(p.id)})));setAccounts(acct??[]);setApprovals(appr??[]);setNeedsReview(Number(intakeCount.count||0)+Number(statementCount.count||0)+Number(documentCount.count||0));
...
const s=StyleSheet.create({center:{flex:1,alignItems:"center",justifyContent:"center"},project:{gap:10},projectCode:{fontSize:9,color:"#1771ab",fontWeight:"900"},projectName:{fontSize:15,fontWeight:"800",color:"#16374f",marginTop:2},projectValue:{fontSize:13,fontWeight:"900",color:"#0a4265"},projectFoot:{flexDirection:"row",justifyContent:"space-between",gap:7},review:{minHeight:64,borderRadius:17,backgroundColor:"#fff1d4",borderWidth:1,borderColor:"#f2d894",padding:13,flexDirection:"row",alignItems:"center",justifyContent:"space-between"},reviewTitle:{fontSize:12,fontWeight:"900",color:"#7d5707"},reviewCopy:{fontSize:9,color:"#9b781f",marginTop:3},reviewArrow:{fontSize:25,color:"#9a7516"},add:{flexDirection:"row",gap:12,alignItems:"center",backgroundColor:"#073f65",padding:15,borderRadius:18,marginTop:4},addPlus:{fontSize:30,color:"white"},addTitle:{fontSize:14,fontWeight:"900",color:"white"},addCopy:{fontSize:10,color:"#c7dae7",marginTop:3,maxWidth:280}});
