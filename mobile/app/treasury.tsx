...
export default function Treasury(){
  ...
  const load=useCallback(async()=>{const w=await loadWorkspace();const [{data:a,error:ae},{data:t,error:te}]=await Promise.all([
    ...
    supabase.from("transfer_pairs").select("id,amount,transfer_date,status,from_account_id,to_account_id,from_project_id,to_project_id,creates_due_to_from,created_at").eq("company_id",w.membership.company_id).order("transfer_date",{ascending:false}).limit(20),
  ]);if(ae)throw ae;if(te)throw te;setAccounts(a??[]);setTransfers(t??[]);setLoading(false);setRefreshing(false)},[]);
  ...
  const known=accounts.filter(a=>a.current_balance!==null&&a.current_balance!==undefined);const total=known.reduce((n,a)=>n+Number(a.current_balance||0),0);const byId=new Map(accounts.map(a=>[a.id,a]));
  return <SafeAreaView style={b.screen} edges={["top"]}><ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={()=>{setRefreshing(true);load()}}/>} contentContainerStyle={b.content}>
    ...
    <SectionHead title="Recent internal movements"/>{transfers.length?transfers.map(t=>{const from=byId.get(t.from_account_id),to=byId.get(t.to_account_id);return <Card key={t.id}><View style={b.row}><View style={{flex:1}}><Text style={s.name}>{t.creates_due_to_from?"Inter-project movement":"Internal transfer"}</Text><Text style={s.note}>{from?.account_name||"Source"} → {to?.account_name||"Destination"} · {t.transfer_date} · {t.status}</Text></View><Text style={s.amount}>{money(t.amount)}</Text></View></Card>}):<Card><Text style={b.muted}>No recorded internal transfers yet.</Text></Card>}
