...
export default function NewTransaction(){
  ...
  const [accounts,setAccounts]=useState<any[]>([]),[projects,setProjects]=useState<any[]>([]);
  ...
  useEffect(()=>{(async()=>{try{const w=await loadWorkspace();const[a,p]=await Promise.all([
    supabase.from("financial_accounts").select("id,institution_name,account_name,current_balance,account_type").eq("company_id",w.membership.company_id).eq("is_active",true).order("account_name"),
  ...
  ]);if(a.error)throw a.error;if(p.error)throw p.error;setAccounts(a.data??[]);setProjects(p.data??[]);if((a.data??[]).length===1)setAccountId(a.data![0].id);setLoading(false)}catch(e){setLoadError(readableError(e));setLoading(false)}})()},[]);
  ...
  return <SafeAreaView style={b.screen} edges={["top"]}><ScrollView contentContainerStyle={b.content} keyboardShouldPersistTaps="handled">
    ...
    {!loading&&!accounts.length&&<Card><Text style={s.emptyTitle}>Add an account first</Text><Text style={s.emptyCopy}>Money cannot be recorded until we know where it moved.</Text><Pressable style={s.smallButton} onPress={()=>router.push("/new-account")}><Text style={s.smallButtonText}>Add account</Text></Pressable></Card>}
    <Card style={s.form}>
      ...
      <Text style={s.label}>Financial account</Text>{accounts.map(a=><Pressable key={a.id} onPress={()=>setAccountId(a.id)} style={[s.pick,accountId===a.id&&s.pickActive]}><View style={{flex:1}}><Text style={s.pickTitle}>{a.institution_name||"Account"} · {a.account_name}</Text><Text style={s.pickMeta}>{String(a.account_type||"account").replaceAll("_"," ")} · recorded {money(a.current_balance)}</Text></View><Text style={s.radio}>{accountId===a.id?"●":"○"}</Text></Pressable>)}
      ...
      {!!selectedAccount&&<View style={s.note}><Text style={s.noteTitle}>Posting to {selectedAccount.account_name}</Text><Text style={s.noteCopy}>This is an accounting record, not just a note. If it is wrong later, correct it through the audit/reversal workflow rather than deleting history.</Text></View>}
