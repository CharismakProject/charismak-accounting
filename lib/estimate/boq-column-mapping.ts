...
export function matchBoqColumnHeader(value: unknown): BoqColumnKey | null {
  ...
  // Soft matches are deliberately conservative so descriptions are not misclassified.
...
export function detectBoqHeaderRow(rows: unknown[][], maxScanRows = 40): DetectedBoqHeader | null {
  ...
  rows.slice(0, maxScanRows).forEach((row, rowIndex) => {
    ...
    // Description is the anchor. A BOQ may be priced or unpriced, so rate/amount are optional.
    ...
    // Require at least Description plus one meaningful BOQ field.
    const hasBoqShape =
