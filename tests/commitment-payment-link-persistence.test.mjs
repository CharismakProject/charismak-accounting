...
test("linked commitments cannot be edited behind their payment ledger",()=>{assert.match(sql,/Void active payment links before changing the commitment cost code/i);assert.match(sql,/Void active payment links before cancelling the commitment/i);assert.match(sql,/Paid amount cannot be below confirmed payment-link allocations/i);});
