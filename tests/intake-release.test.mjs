...
test("V6 does not send PDF route to legacy binary edge parser",()=>{const s=fs.readFileSync(new URL("../app/add/UniversalIntakeV6.tsx",import.meta.url),"utf8");const pdfBlock=s.slice(s.indexOf("async function pdfAnalyse"),s.indexOf("async function removeResult"));assert.doesNotMatch(pdfBlock,/analyse-intake-document-v3/);assert.match(pdfBlock,/browser_pdf_text_v1/)});
test("client PDF text reader uses pdf.js text content page-by-page",()=>{const s=fs.readFileSync(new URL("../app/add/client-ocr.ts",import.meta.url),"utf8");assert.match(s,/getTextContent/);assert.match(s,/for\(let i=1;i<=pdfDoc\.numPages;i\+\+\)/);assert.match(s,/MAX_TEXT=650_000/)});
