...
test("versioning supersedes prior approval atomically before inserting the next approved row",()=>{const supersede=sql.indexOf("set status='superseded'");const insert=sql.indexOf("insert into public.project_progress_valuations(company_id");assert.ok(supersede>0);assert.ok(insert>supersede);assert.match(sql,/valuation_date_value > current_date[\s\S]*cannot be in the future/i);assert.match(sql,/valuation_date_value < previous_valuation_date[\s\S]*cannot be earlier/i);});
...
test("progress draft never posts Money, commitments, forecast or billing records",()=>{assert.doesNotMatch(sql,/insert\s+into\s+public\.transactions/i);assert.doesNotMatch(sql,/update\s+public\.transactions/i);assert.doesNotMatch(sql,/insert\s+into\s+public\.project_cost_commitments/i);assert.doesNotMatch(sql,/update\s+public\.project_cost_commitments/i);assert.doesNotMatch(sql,/insert\s+into\s+public\.project_cost_forecasts/i);assert.doesNotMatch(sql,/update\s+public\.project_cost_forecasts/i);assert.doesNotMatch(sql,/insert\s+into\s+public\.journal_/i);});
