...
test("unknown commitments and forecast are presented as unavailable while forecast extension is disabled",()=>{assert.match(web,/commitments\s*:\s*forecastEnabled\s*\?[\s\S]*?\:\s*null/);assert.match(web,/forecastCostToComplete\s*:\s*forecastEnabled\s*\?[\s\S]*?\:\s*null/);assert.match(mobile,/Unpaid Commitments/);assert.match(mobile,/Forecast Final Cost/);});
