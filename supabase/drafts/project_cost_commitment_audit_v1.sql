-- DRAFT ONLY — apply after project_cost_commitment_forecast_v1.sql.
-- Adds immutable before/after history for commitment creates, edits and payment-link effects.
...
create table if not exists public.project_cost_commitment_revisions (
  ...
  revision_type text not null check (revision_type in ('created','updated','payment_linked','payment_unlinked')),
  before_data jsonb,
...
create index if not exists project_cost_commitment_revisions_commitment_idx on public.project_cost_commitment_revisions(commitment_id,changed_at desc);
alter table public.project_cost_commitment_revisions enable row level security;
create policy project_cost_commitment_revisions_read on public.project_cost_commitment_revisions for select to authenticated using (private.can_view_project_cost(project_id));
revoke all on table public.project_cost_commitment_revisions from anon;
revoke insert,update,delete,truncate,references,trigger on table public.project_cost_commitment_revisions from authenticated;
...
begin
  ...
  if commitment_id is null then
    ...
    select to_jsonb(c) into before_snapshot from public.project_cost_commitments c where c.id=commitment_id and c.project_id=target_project_id and c.company_id=company for update;
    ...
    select to_jsonb(c) into after_snapshot from public.project_cost_commitments c where c.id=result_id;
    insert into public.project_cost_commitment_revisions(commitment_id,project_id,revision_type,before_data,after_data,reason,changed_by)
    values(result_id,target_project_id,'updated',before_snapshot,after_snapshot,'Commitment details updated.',actor);
  end if;
  return result_id;
end;
$$;
