...
function parseDetailSheet(sheet:ParsedWorkbookSheet,header:DetectedBoqHeader,warnings:BoqImportWarning[]):ImportedBoqSection[]|null{
  ...
  for(let r=header.rowIndex+1;r<sheet.rows.length;r++){
    ...
    const description=text(cell(row,header.columns.description));const serial=text(cell(row,header.columns.serial));const rowSection=text(cell(row,header.columns.section));const unitRaw=text(cell(row,header.columns.unit));const qtyRaw=number(cell(row,header.columns.quantity));const rate=number(cell(row,header.columns.rate));const amountRaw=number(cell(row,header.columns.amount));const combined=description||values.join(" ");const hasMeasuredIdentity=Boolean(description)&&(qtyRaw!==null||Boolean(unitRaw)||rate!==null||Boolean(serial));
