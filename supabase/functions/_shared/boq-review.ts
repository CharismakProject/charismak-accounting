...
export type BoqReviewSuggestion = {costCode:string|null;costCodeName:string|null;recipeFamily:BoqRecipeFamily;recipeLabel:string;supplyResponsibility:BoqSupplyResponsibility;confidence:BoqReviewConfidence;requiresAttention:boolean;reasons:string[]};
export type ReviewableBoqItem = {description:string;unit?:string;quantity?:number};
export type ReviewableBoqSection = {title:string;context?:string[];items:Array<ReviewableBoqItem & {reviewSuggestion?:BoqReviewSuggestion}>};

export const REVIEW_COST_CODES = [["01","Preliminaries"],["02","Substructure"],["03","Concrete & Reinforcement"],["04","Blockwork & Masonry"],["05","Structural Steel"],["06","Roofing"],["07","Doors"],["08","Windows & Glazing"],["09","Plastering & Screeding"],["10","Floor Finishes"],["11","Wall Finishes"],["12","Ceilings"],["13","Painting & Decoration"],["14","Joinery & Fixtures"],["15","Plumbing & Sanitary"],["16","Electrical"],["17","Mechanical & HVAC"],["18","External Works"],["19","Plant, Equipment & Specialist Works"],["20","Professional, Statutory & Other"]] as const;
...
function directSupply(text:string):boolean{return includesAny(text,["door","window","glazing","glass","sanitary ware","sanitaryware","water closet"," wc ","wash hand basin","whb","shower mixer","mixer tap","sink","extractor fan","water heater","socket","switch","light fitting","light fixture","luminaire","air conditioner","ac unit","wardrobe","kitchen cabinet","cabinet","ironmongery"]);}
...
function suggestRecipe(text:string):{family:BoqRecipeFamily;label:string;code:string|null;strong:boolean}{
  ...
  if(includesAny(text,["screeding","floor screed","cement screed","screed to","levelling screed","leveling screed"]))return{family:"screeding",label:"Screeding recipe",code:"09",strong:true};
  if(includesAny(text,["floor tile","floor tiling","field tiling","stone tiling","marble tiling","porcelain floor","ceramic floor","granite floor","marble floor"]))return{family:"floor_tiling",label:"Floor tiling recipe",code:"10",strong:true};
  ...
  if(includesAny(text,["painting","paint to","paint area","emulsion paint","gloss paint","textured paint","primer coat"]))return{family:"painting",label:"Painting recipe",code:"13",strong:true};
  ...
  if(directSupply(` ${text} `)){let code:string|null=null;if(includesAny(text,["door","ironmongery"]))code="07";else if(includesAny(text,["window","glazing","glass"]))code="08";else if(includesAny(text,["wardrobe","kitchen cabinet","cabinet"]))code="14";else if(includesAny(text,["sanitary","water closet"," wc ","wash hand basin","whb","shower","sink","mixer","water heater"]))code="15";else if(includesAny(text,["socket","switch","light fitting","light fixture","luminaire","extractor fan"]))code="16";else if(includesAny(text,["air conditioner","ac unit"]))code="17";return{family:"direct_supply",label:"Direct supply item",code,strong:true};}
...
function costFromText(text:string,fallback:string|null):string|null{if(fallback)return fallback;const rules:Array<[string,readonly string[]]>=[["17",["hvac","air conditioning","mechanical ventilation","ductwork"]],["16",["electrical","lighting","power installation"]],["15",["plumbing","sanitary","drainage","water supply"]],["14",["joinery","cabinetry","wardrobe","kitchen cabinet"]],["13",["painting","decoration"]],["12",["ceiling","gypsum","pop work","reflective surfaces"]],["11",["wall finish","wall tile","cladding"]],["10",["floor finish","floor tile","flooring","tiling","marble"]],["09",["plaster","render","screed"]],["08",["window","glazing","glass"]],["07",["door","ironmongery"]],["06",["roof","roofing"]],["05",["structural steel","steelwork"]],["04",["blockwork","masonry","walling"]],["03",["concrete","reinforcement","rebar","formwork"]],["02",["substructure","foundation","excavation","earthwork"]],["18",["external works","landscaping","fence","paving"]],["19",["specialist works","equipment","plant"]],["20",["professional fee","statutory","permit","approval fee"]],["01",["preliminaries","prelims","mobilization","mobilisation"]]];return rules.find(([,terms])=>includesAny(text,terms))?.[0]??null;}

type SupplySuggestion={value:BoqSupplyResponsibility;strong:boolean;reason:string};
function explicitItemSupply(text:string):SupplySuggestion|null{
  if(includesAny(text,["labour only","labor only","laying only","lay only","installation only","install only","fixing only","fix only","workmanship only"]))return{value:"labour_only",strong:true,reason:"Item wording explicitly limits the priced work to labour/installation."};
  if(includesAny(text,["client supplied","client supply","by client","free issue","free issued","owner supplied","employer supplied"]))return{value:"client",strong:true,reason:"Item wording says the item/material is client supplied."};
  if(includesAny(text,["nominated subcontractor","nominated supplier","specialist contractor","by specialist","specialist supply"]))return{value:"specialist",strong:true,reason:"Item wording indicates specialist or nominated supply."};
  if(includesAny(text,["supply and install","supply & install","supply and fix","supply and lay","provide and fix","provide and install","supply deliver and install"])||/^supply\b/.test(text))return{value:"contractor",strong:true,reason:"Item wording explicitly makes supply part of the contractor-priced work."};
  return null;
}
function supplySuggestion(itemText:string,contextText:string,recipe:BoqRecipeFamily):SupplySuggestion{
  const itemExplicit=explicitItemSupply(itemText);if(itemExplicit)return itemExplicit;
  if(includesAny(contextText,["labour only","labor only","laying only","lay only","installation only","install only","fixing only","fix only","workmanship only"]))return{value:"labour_only",strong:true,reason:"Section instructions state that these BOQ lines are laying/installation only."};
  if(includesAny(contextText,["client supplied","client supply","by client","free issue","free issued","owner supplied","employer supplied"]))return{value:"client",strong:true,reason:"Section instructions state that the material/item is client or employer supplied."};
  if(includesAny(contextText,["nominated subcontractor","nominated supplier","specialist contractor","by specialist","specialist supply"]))return{value:"specialist",strong:true,reason:"Section instructions identify specialist or nominated supply."};
  ...
  return{value:"unknown",strong:false,reason:"Supply responsibility is not clear from the item or section instructions."};
}

export function suggestBoqItemReview(sectionTitle:string,item:ReviewableBoqItem,sectionContext:readonly string[]=[]):BoqReviewSuggestion{
  ...
  if(code&&recipe.family!=="needs_review"&&supply.value!=="unknown")confidence=recipe.strong&&supply.strong?"high":"medium";else if(code&&recipe.strong)confidence="medium";
