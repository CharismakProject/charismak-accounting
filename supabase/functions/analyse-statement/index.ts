...
type Row={date:string;valueDate:string|null;narration:string;reference:string|null;counterparty:string|null;debit:number|null;credit:number|null;amount:number|null;balance:number|null;source:number;confidence:number};
const k=(v:unknown)=>String(v??"").trim().toLowerCase().replace(/ngn|naira/g,"").replace(/[^a-z0-9]/g,"");
const money=(v:unknown)=>{if(typeof v==="number"&&Number.isFinite(v))return v;let s=String(v??"").trim();if(!s)return null;const dr=/\bDR\b/i.test(s)||/^\(.*\)$/.test(s);const cr=/\bCR\b/i.test(s);s=s.replace(/[₦,$]/g,"").replace(/NGN|\bDR\b|\bCR\b/gi,"").replace(/^N(?=\s*\d)/i,"").replace(/[()\s]/g,"");const n=Number(s);if(!Number.isFinite(n))return null;return dr?-Math.abs(n):cr?Math.abs(n):n};
function date(v:unknown){if(v instanceof Date&&!isNaN(v.getTime()))return v.toISOString().slice(0,10);if(typeof v==="number"){const p=XLSX.SSF.parse_date_code(v);if(p)return `${p.y}-${String(p.m).padStart(2,"0")}-${String(p.d).padStart(2,"0")}`;}const s=String(v??"").trim();let m=s.match(/^(20\d{2})[-/](\d{1,2})[-/](\d{1,2})/);if(m)return `${m[1]}-${m[2].padStart(2,"0")}-${m[3].padStart(2,"0")}`;m=s.match(/^(\d{1,2})[/-](\d{1,2})[/-](\d{2,4})/);if(m){const y=m[3].length===2?2000+Number(m[3]):Number(m[3]);return `${y}-${m[2].padStart(2,"0")}-${m[1].padStart(2,"0")}`;}m=s.match(/^(\d{1,2})[-\s]([A-Za-z]{3})[-\s](\d{2,4})/);if(m&&months[m[2].toLowerCase()]){const y=m[3].length===2?2000+Number(m[3]):Number(m[3]);return `${y}-${String(months[m[2].toLowerCase()]).padStart(2,"0")}-${m[1].padStart(2,"0")}`;}return null;}
const A={date:new Set(["transactiondate","date","transdate","postingdate","posteddate","transactiontime","transactiondatetime"]),value:new Set(["valuedate","effectivedate"]),narr:new Set(["description","narration","transactiondetails","details","remarks","remark","memo","transactiondescription"]),ref:new Set(["transactionreference","reference","referenceno","referencenumber","ref","transactionref"]),party:new Set(["counterparty","beneficiary","payer","payee","sender","recipient"]),debit:new Set(["debit","debitamount","withdrawal","withdrawals","moneyout"]),credit:new Set(["credit","creditamount","deposit","deposits","moneyin"]),amount:new Set(["amount","transactionamount","debitcredit","debitcreditamount","transactionvalue"]),balance:new Set(["balance","runningbalance","closingbalance","availablebalance","balanceaftertransaction","balanceafter","accountbalance","walletbalance","availablebalanceaftertransaction","postbalance"]),type:new Set(["type","transactiontype","drcr","debitcreditindicator"])};
const col=(h:unknown[],s:Set<string>)=>h.findIndex(x=>s.has(k(x)));
function spreadsheet(bytes:Uint8Array){const wb=XLSX.read(bytes,{type:"array",cellDates:true,raw:false});let best:any=null;for(const sn of wb.SheetNames){const rs=XLSX.utils.sheet_to_json(wb.Sheets[sn],{header:1,defval:"",raw:false,blankrows:false}) as unknown[][];for(let i=0;i<Math.min(rs.length,80);i++){const h=rs[i]??[];const score=(col(h,A.date)>=0?3:0)+(col(h,A.narr)>=0?3:0)+((col(h,A.amount)>=0||col(h,A.debit)>=0||col(h,A.credit)>=0)?3:0)+(col(h,A.balance)>=0?1:0);if(!best||score>best.score)best={rs,i,score,sn};}}if(!best||best.score<6)throw new Error("Transaction header not found. Expected Date, Description and Amount/Debit/Credit.");const h=best.rs[best.i],dc=col(h,A.date),vc=col(h,A.value),nc=col(h,A.narr),rc=col(h,A.ref),pc=col(h,A.party),dbc=col(h,A.debit),crc=col(h,A.credit),ac=col(h,A.amount),bc=col(h,A.balance),tc=col(h,A.type);const rows:Row[]=[];for(let i=best.i+1;i<best.rs.length;i++){const r=best.rs[i],d=date(r[dc]);if(!d)continue;const db=dbc>=0?money(r[dbc]):null,cr=crc>=0?money(r[crc]):null,am=ac>=0?money(r[ac]):null;let x:number|null=null;let debit:number|null=null,credit:number|null=null;if(cr!==null&&cr!==0){x=Math.abs(cr);credit=Math.abs(cr);}else if(db!==null&&db!==0){x=-Math.abs(db);debit=Math.abs(db);}else if(am!==null){const t=tc>=0?String(r[tc]??"").toLowerCase():"";if(/debit|withdraw|\bdr\b/.test(t)){x=-Math.abs(am);debit=Math.abs(am);}else if(/credit|deposit|\bcr\b/.test(t)){x=Math.abs(am);credit=Math.abs(am);}else x=am;}if(x===null)continue;rows.push({date:d,valueDate:vc>=0?date(r[vc]):null,narration:nc>=0?String(r[nc]??"").trim()||"Statement transaction":"Statement transaction",reference:rc>=0?String(r[rc]??"").trim()||null:null,counterparty:pc>=0?String(r[pc]??"").trim()||null:null,debit,credit,amount:x,balance:bc>=0?money(r[bc]):null,source:i+1,confidence:.99});}if(!rows.length)throw new Error("No valid transaction rows found in spreadsheet.");return {parser:"spreadsheet_v4",confidence:.99,warnings:bc>=0?[]:["No statement balance column was recognised; transactions were imported without changing the recorded bank balance."],rows};}

const DATE_TOKEN="(?:20\\d{2}[-/]\\d{1,2}[-/]\\d{1,2}|\\d{1,2}[/-]\\d{1,2}[/-]\\d{2,4}|\\d{1,2}[-\\s](?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[-\\s]\\d{2,4})";
const MONEY_TOKEN=/\(?\s*(?:NGN|₦|N)?\s*-?\d{1,3}(?:,\d{3})*(?:\.\d{2})\s*\)?\s*(?:DR|CR)?/gi;
function dateMatches(s:string){return [...s.matchAll(new RegExp(DATE_TOKEN,"gi"))].map(m=>({raw:m[0],index:m.index??0,value:date(m[0])})).filter(m=>Boolean(m.value));}
function moneyMatches(s:string){return [...s.matchAll(MONEY_TOKEN)].map(m=>({raw:m[0],index:m.index??0,value:money(m[0])})).filter(m=>m.value!==null) as {raw:string;index:number;value:number}[];}
