...
export default function LoginPage() {
  ...
  const [intent, setIntent] = useState<"owner"|"team"|"">("");
  ...
  useEffect(()=>{
    ...
    if(nextIntent==="owner"||nextIntent==="team") setIntent(nextIntent);
  },[]);
  ...
  async function submit(event: FormEvent) {
    ...
    if (mode === "login") {
      ...
      const { data, error } = await supabase.auth.signUp({
        ...
        password,
        options: {
          data: { full_name: fullName, onboarding_intent: intent || null },
          emailRedirectTo: typeof window !== "undefined" ? `${window.location.origin}/welcome` : undefined,
        },
      });
      if (error) setMessage(error.message);
      else if (!data.session) {
        setMessage(intent==="team"
          ? "Account created. Confirm this exact email address. If your MD invited it, Charismak will attach the approved company role and projects automatically."
          : "Account created. Confirm your email, then continue from the Welcome page to set up your company.");
        setMode("login");
      } else {
        router.push(intent==="team"?"/onboarding/role":"/welcome");
        router.refresh();
      }
    }
    setBusy(false);
  ...
  async function resendConfirmation() {
    if (!email) { setMessage("Enter your email address first, then click resend confirmation."); return; }
    setResending(true); setMessage("");
    const supabase = createClient();
    const { error } = await supabase.auth.resend({ type: "signup", email, options: { emailRedirectTo: typeof window !== "undefined" ? `${window.location.origin}/welcome` : undefined } });
      ...
      <section style={{ width: "min(460px,100%)", background: "white", border: "1px solid #e1e8ef", borderRadius: 22, padding: 30, boxShadow: "0 20px 60px rgba(15,36,58,.10)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 24 }}>
          <div style={{ width: 56, height: 56, borderRadius: "50%", background: "white", border: "1px solid #dce4ec", display: "grid", placeItems: "center", position: "relative", overflow: "visible" }}>
            <div style={{ width: 48, height: 48, borderRadius: "50%", overflow: "hidden", background: "white", display: "grid", placeItems: "center" }}><img src={LOGO_URL} alt="Charismak" style={{ width: "100%", height: "100%", objectFit: "contain" }} /></div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", background: "#f3f6f9", padding: 4, borderRadius: 12, marginBottom: 22 }}>
          ...
          <button onClick={() => { setMode("signup"); setMessage(""); }} style={{ border: 0, borderRadius: 9, padding: 10, background: mode === "signup" ? "white" : "transparent", fontWeight: 800 }}>Create login</button>
        </div>

        {mode==="signup"&&<p style={{margin:"-8px 0 14px",padding:10,borderRadius:10,background:"#eef7f4",color:"#326654",fontSize:11,lineHeight:1.5}}><b>A login is not an MD role.</b> After authentication, Charismak either creates an owner workspace through onboarding or joins you to an invitation sent by an MD.</p>}
        ...
        <form onSubmit={submit} style={{ display: "grid", gap: 14 }}>
          ...
          <button disabled={busy} type="submit" style={{ border: 0, borderRadius: 12, padding: 13, background: "#0b3253", color: "white", fontWeight: 850 }}>{busy ? "Please wait…" : mode === "login" ? "Sign in" : "Create login"}</button>
          {mode === "login" && <button type="button" disabled={resending} onClick={resendConfirmation} style={{ border: "1px solid #cfd9e3", borderRadius: 12, padding: 11, background: "white", color: "#0b3253", fontWeight: 800 }}>{resending ? "Resending…" : "Resend confirmation email"}</button>}
        </form>
      </section>
    </main>
  );
