...
export default async function ReviewInbox({searchParams}:{searchParams:Promise<{deleted?:string}>}){
  ...
  const projectDocIds=new Set((docs??[]).map((r:any)=>r.document_id));const intakeOnly=(intake??[]).filter((r:any)=>!projectDocIds.has(r.document_id));const total=intakeOnly.length+(statements??[]).length+(docs??[]).length;

  return <main className="simple-shell"><div className="simple-wrap">
    <div className="simple-top"><Link href="/">← Home</Link><span style={{display:"flex",gap:12}}><Link href="/documents">Documents</Link><Link href="/add">+ Add</Link></span></div>
    ...
    {q.deleted&&<div className="smart-success">Record deleted. The deletion remains in the audit trail.</div>}
