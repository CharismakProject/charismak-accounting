...
export default function BoqProjectPreviewClient({boq,summary,decisions,sourceReviewIssue}:{boq:SectionedBoq;summary:EstimateSummary;decisions:ReviewedBoqDecisionMap;sourceReviewIssue?:string|null}){
  ...
  const readyToStage=preview.readyToStage&&!sourceReviewIssue;

  function downloadSnapshot(){if(!readyToStage)return;const blob=new Blob([serializeProjectCreationPreview(preview)],{type:"application/json;charset=utf-8"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download=`${safeName(projectName)}-project-stage.json`;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);}
  function stageWorkspace(){if(!readyToStage)return;try{const workspace=buildStagedProjectWorkspace(preview);saveStagedProjectWorkspace(workspace);router.push(`/projects/draft/${encodeURIComponent(workspace.workspaceId)}`);}catch(error){window.alert(error instanceof Error?error.message:"Could not stage this project workspace.");}}
  ...
  return <section style={{marginTop:14,border:"1px solid #cbdde7",borderRadius:14,overflow:"hidden",background:"#fff"}}>
    ...
    {open&&<div style={{padding:15,display:"grid",gap:13}}>
      {sourceReviewIssue&&<div style={{background:"#fff4ce",borderRadius:10,padding:11,color:"#775c18",fontSize:10,lineHeight:1.55}}><b>Source document review required:</b> {sourceReviewIssue} Project staging stays disabled until this source-level review is explicitly cleared.</div>}
