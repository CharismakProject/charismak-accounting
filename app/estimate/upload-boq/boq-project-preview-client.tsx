...
export default function BoqProjectPreviewClient({boq,summary,decisions}:{boq:SectionedBoq;summary:EstimateSummary;decisions:ReviewedBoqDecisionMap}){
  ...
  const preview=useMemo(()=>buildProjectCreationPreview({
    boq,summary,decisions,
    choice:{
      projectName,
      internalBudgetBasis:internalBasis,
      contractValueBasis:contractBasis,
      explicitInternalBudget:numeric(explicitBudget),
      explicitContractValue:numeric(explicitContract),
    },
  }),[boq,summary,decisions,projectName,internalBasis,contractBasis,explicitBudget,explicitContract]);

  function downloadSnapshot(){
    if(!preview.readyToStage)return;
    const blob=new Blob([serializeProjectCreationPreview(preview)],{type:"application/json;charset=utf-8"});
    const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download=`${safeName(projectName)}-project-stage.json`;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);
  }

  return <section style={{marginTop:14,border:"1px solid #cbdde7",borderRadius:14,overflow:"hidden",background:"#fff"}}>
    <div style={{padding:15,display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",flexWrap:"wrap",background:"#eef5f9"}}>
      <div><small style={{fontSize:9,fontWeight:900,letterSpacing:".1em",color:"#0b668f"}}>CREATE PROJECT · REVIEW STAGE</small><h3 style={{margin:"5px 0 3px",fontSize:16,color:"#173f5a"}}>Use this estimate as a project foundation</h3><p style={{margin:0,fontSize:10,lineHeight:1.5,color:"#687d8c",maxWidth:700}}>Nothing is created yet. First choose the internal cost budget and contract value separately. Overhead, profit and VAT are never silently copied into the cost budget.</p></div>
      <button type="button" onClick={()=>setOpen(value=>!value)} style={open?secondary:primary}>{open?"Close review":"Prepare Project"}</button>
    </div>
    ...
    {open&&<div style={{padding:15,display:"grid",gap:13}}>
      ...
      {preview.issues.length>0?<div style={{background:"#fff4ce",borderRadius:10,padding:11,color:"#775c18",fontSize:10,lineHeight:1.55}}><b>Before this can be staged:</b><ul style={{margin:"5px 0 0",paddingLeft:18}}>{preview.issues.slice(0,12).map(issue=><li key={issue}>{issue}</li>)}{preview.issues.length>12&&<li>+ {preview.issues.length-12} more review issues</li>}</ul></div>:<div style={{background:"#e7f6ef",borderRadius:10,padding:11,color:"#176247",fontSize:10,lineHeight:1.5}}><b>Ready to stage.</b> Project name, cost codes, reviewed BOQ rates, supply responsibility, internal budget and contract-value basis are complete.</div>}

      <div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"center",flexWrap:"wrap",borderTop:"1px solid #e5ecef",paddingTop:12}}><small style={{fontSize:9,lineHeight:1.45,color:"#718391",maxWidth:690}}>This button creates only a versioned review snapshot file. It does not insert a project, apply a database migration, or post a budget to Accounting. Live creation comes only after the destination schema/permissions bridge is approved.</small><button type="button" disabled={!preview.readyToStage} onClick={downloadSnapshot} style={{...primary,opacity:preview.readyToStage?1:.45}}>Stage reviewed project snapshot</button></div>
    </div>}
  </section>;
}
