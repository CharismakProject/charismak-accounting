...
export default async function CommitmentPaymentPage({params}:{params:Promise<{id:string}>}){
  ...
  const txMap=new Map((transactions??[]).map((r:any)=>[r.id,r]));const commitmentMap=new Map((commitments??[]).map((r:any)=>[r.id,r]));
  const activeLinks=(links??[]).map((r:any)=>({id:r.id,transactionId:r.transaction_id,transactionTitle:txMap.get(r.transaction_id)?.title??"Money payment",commitmentId:r.commitment_id,commitmentDescription:commitmentMap.get(r.commitment_id)?.description??"Commitment",allocatedAmount:Number(r.allocated_amount)}));
