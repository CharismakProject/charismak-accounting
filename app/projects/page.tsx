...
import StagedProjectsClient from "./staged-projects-client";
...
export default async function ProjectsPage(){
  ...
  return <main className="page-canvas"><div className="page-wrap">
    <div className="page-toolbar project-toolbar"><div style={{display:"flex",gap:10,flexWrap:"wrap"}}><Link href="/" className="back-link">← Home</Link><Link href="/add" className="secondary-link">+ Add records</Link><Link href="/projects/estimator-review" className="secondary-link">Review Estimator BOQ</Link></div><Link href="/projects/new" className="md-button">+ New Project</Link></div>
    <header className="page-heading compact project-list-heading"><p className="page-eyebrow">Projects</p><h1>Your Project Portfolio</h1><p>Reviewed Estimate drafts can appear here before approval; live projects continue to show actual scope, money, documents, commitments and decisions.</p></header>
    <StagedProjectsClient/>
    <section className="project-summary-strip">
      <div><small>Live projects</small><b>{rows.length}</b><span>{active} active / on hold</span></div>
      <div><small>Identified commercial value</small><b>{money(totals.commercial)}</b><span>Base + additions + variations</span></div>
      <div><small>Client project funding</small><b>{money(totals.funding)}</b><span>Across permitted live projects</span></div>
    ...
    </article>})}{rows.length===0&&<article className="compact-card"><b>No live projects yet.</b><p style={{marginBottom:0,color:"#718195"}}>Create a project, stage one from Estimate, or use Add to let Charismak organise existing records.</p></article>}</section>
  </div></main>;
}
