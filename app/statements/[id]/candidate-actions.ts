...
export async function linkCandidateToProject(formData:FormData){
  const {supabase}=await requireUser(); const candidateId=String(formData.get("candidate_id")||""); const projectId=String(formData.get("project_id")||""); const importId=String(formData.get("import_id")||""); const candidateName=String(formData.get("candidate_name")||"Project");
  if(!candidateId||!projectId||!importId)throw new Error("Candidate, project and statement import are required.");
  const {data:linked,error}=await supabase.rpc("link_statement_candidate",{candidate_id:candidateId,target_project:projectId}); if(error)throw new Error(`Could not link candidate: ${error.message}`);
  ...
  revalidatePath(`/statements/${importId}`);revalidatePath(`/statements/${importId}/projects`);revalidatePath(`/projects/${projectId}`);revalidatePath("/projects");revalidatePath("/");
  const reclassified=Number((linked as any)?.reclassified_rows??0); const autoPosted=Number((posting as any)?.autoPosted??0);
  redirect(`/statements/${importId}/projects?linked=${encodeURIComponent(candidateName)}&reclassified=${reclassified}&autoposted=${autoPosted}`);
}
...
export async function ignoreCandidate(formData:FormData){
  ...
  revalidatePath(`/statements/${importId}`);revalidatePath(`/statements/${importId}/projects`);redirect(`/statements/${importId}/projects?ignored=1`);
}
