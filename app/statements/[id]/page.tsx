...
export default async function StatementReviewPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<Record<string,string|string[]|undefined>>}){
  ...
  const {data:authData}=await supabase.auth.getUser(); if(!authData.user)redirect("/welcome");
  ...
  const [{data:queue,error:queueError},{data:reviewCountRaw},{data:postedCountRaw},{data:knownCountRaw}]=await Promise.all([
    supabase.rpc("statement_review_queue",{target_import:id,target_view:view,target_keyword:keyword||null,target_limit:PAGE_SIZE,target_offset:offset}),
    supabase.rpc("statement_review_queue",{target_import:id,target_view:"review",target_keyword:null,target_limit:1,target_offset:0}),
    supabase.rpc("statement_review_queue",{target_import:id,target_view:"posted",target_keyword:null,target_limit:1,target_offset:0}),
    supabase.rpc("statement_review_queue",{target_import:id,target_view:"known",target_keyword:null,target_limit:1,target_offset:0}),
  ]);
  if(queueError)throw new Error(queueError.message);
  const rowIds=(queue??[]).map((r:any)=>r.row_id); const totalFiltered=Number((queue??[])[0]?.total_count??0);
  const reviewCount=Number((reviewCountRaw??[])[0]?.total_count??0); const postedCount=Number((postedCountRaw??[])[0]?.total_count??0); const knownCount=Number((knownCountRaw??[])[0]?.total_count??0);
  let rows:any[]=[];
  if(rowIds.length){
  ...
  const [{data:projects},{data:discoveryRaw}]=await Promise.all([
    supabase.from("projects").select("id,project_code,name").eq("company_id",statement.company_id).in("status",["draft","active","on_hold"]).order("name"),
    Number((statement as any).rows_total??0)>0?supabase.rpc("statement_project_discovery_summary",{target_import:id}):Promise.resolve({data:null} as any),
  ]);
  ...
  const parserExceptions=Number((statement as any).rows_need_review??0);
  ...
  return <main className="page-shell"><div className="content-wrap review-wrap">
    <div className="page-actions"><div className="button-row"><Link href="/" className="text-link">← Dashboard</Link><Link href="/statements" className="text-link">Statement History</Link></div><div className="button-row"><Link href={`/statements/${id}/projects`} className="secondary-button">Project Signals</Link><Link href="/add" className="primary-link-button">Upload Next Record</Link></div></div>
    <header className="compact-header"><p className="mini-eyebrow">Statement Review</p><h1>{(statement as any).detected_institution_name||"Financial Statement"} · {(statement as any).detected_account_name||"Account"}</h1><p>{document?.file_name} · {(statement as any).period_start||"Period unknown"} → {(statement as any).period_end||"—"}</p></header>
    ...
    {postedCount>0&&<div className="notice notice-green"><b>{postedCount.toLocaleString()} transactions already resolved/posted.</b> High-confidence rules, confirmed decisions and matched accounting evidence are reflected here. {reviewCount.toLocaleString()} unresolved transactions remain.</div>}
    ...
    <section className="review-kpis">
      {[["Statement rows",(statement as any).rows_total],["Resolved / posted",postedCount],["Needs action",reviewCount],["Already known",knownCount],["Parser exceptions",parserExceptions]].map(([label,value])=><div className="mini-card" key={String(label)}><small>{label}</small><b>{value}</b></div>)}
    </section>
    ...
    {Number((statement as any).rows_total??0)>0&&<article className="review-card" style={{marginBottom:14}}>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}><Link href={`/statements/${id}/projects`} className="primary-link-button">Review / Create Projects →</Link><Link href="/add" className="secondary-button">Upload another record</Link></div>
    </article>}
    ...
    <article className="review-card" id="transactions">
      ...
      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:10}}><Link href={tabHref("review")} className={view==="review"?"primary-link-button":"secondary-button"}>Needs Action ({reviewCount})</Link><Link href={tabHref("posted")} className={view==="posted"?"primary-link-button":"secondary-button"}>Posted ({postedCount})</Link><Link href={tabHref("known")} className={view==="known"?"primary-link-button":"secondary-button"}>Already Known ({knownCount})</Link></div>
