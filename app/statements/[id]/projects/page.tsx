...
export default async function StatementProjectDiscoveryPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<Record<string,string|string[]|undefined>>}){
  ...
  return <main className="page-canvas"><div className="page-wrap" style={{maxWidth:1050}}>
    ...
    {createdCode&&<div className="notice notice-green" style={{marginBottom:12}}><b>{createdCode} created and linked.</b> Historical rows carrying that project tag were attached immediately. {reclassified>0&&`${reclassified.toLocaleString()} previously reconciliation-only rows were reclassified into the project. `}{autoPosted>0&&`${autoPosted.toLocaleString()} additional high-confidence rows were posted automatically.`}</div>}
    {linkedCode&&<div className="notice notice-green" style={{marginBottom:12}}><b>Project signal linked.</b> {linkedCode} is now connected to the selected existing project. {reclassified>0&&`${reclassified.toLocaleString()} historical reconciliation-only rows were moved into that project.`}</div>}
    ...
    <section style={{display:"grid",gap:9}}>
      ...
      {projectCandidates.length===0&&<article className="compact-card"><b>No unreviewed project signals.</b><p style={{margin:"5px 0 0",fontSize:10,color:"#718195"}}>All currently detected tags have been created, linked or ignored.</p></article>}
      {projectCandidates.map((c:any)=>{const e=c.evidence??{};const proposedCode=String(c.suggested_code||c.suggested_name||e.keyword||"").toUpperCase();const createHref=`/projects/new?candidate=${encodeURIComponent(c.id)}&import=${encodeURIComponent(id)}&name=${encodeURIComponent(c.suggested_name||e.keyword||"")}&code=${encodeURIComponent(proposedCode)}${managedClient?`&client=${encodeURIComponent(managedClient)}`:""}`;return <article key={c.id} className="compact-card" style={{display:"grid",gap:9}}>
        ...
        <p style={{margin:0,fontSize:9,color:"#64788b"}}>Creating or linking this tag will attach its historical statement rows to the chosen project. Rows you deliberately classified as personal, company-level or internal transfer are not overwritten.</p>
