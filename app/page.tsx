...
export default async function Home() {
  ...
  const [projectResult, accountResult, approvalResult, statementResult, transactionResult, auditResult] = await Promise.all([
    ...
    supabase.from("canonical_transactions").select("id,project_id,signed_amount,classification,status,transaction_date,narration,category_name").eq("company_id", membership.company_id).order("transaction_date", { ascending: false }).limit(1200),
  ...
  return <main className={`role-shell role-${roleAccent}-shell`}>
    ...
    <section className="role-main">
      ...
      {active === "md_owner" && <MdDashboard projects={projects} totals={totals} bankCash={bankCash} approvals={pendingApprovals} accounts={accounts} statements={statements} auditRows={auditRows} monthly={monthly} />}
      {active === "accountant_cfo" && <FinanceDashboard accounts={accounts} bankCash={bankCash} statements={statements} approvals={pendingApprovals} transactions={transactions} unclassifiedEstimate={unclassifiedEstimate} monthly={monthly} />}
...
function MdDashboard({ projects, totals, bankCash, approvals, accounts, statements, auditRows, monthly }: any) {
  ...
  return <><DashboardHeader eyebrow="Executive overview" title="Company Control Room" subtitle="Live company cash, project position, approvals and statement intelligence." /><div className="role-content">
    ...
    <section className="chart-grid"><LineChart title="Funding & Expenditure Trend" eyebrow="Company cashflow" data={monthly}/><DonutCard title="Portfolio Health" eyebrow="Project position" pct={healthPct} label="healthy" stats={[["Healthy",healthy],["Needs attention",Math.max(projects.length-healthy,0)],["Projects",projects.length]]}/></section>
...
function FinanceDashboard({ accounts, bankCash, statements, approvals, transactions, unclassifiedEstimate, monthly }: any) {
  ...
  const resolvedPct=total?Math.min(100,Math.round((auto+known)/total*100)):0;
  return <><DashboardHeader eyebrow="Finance operations" title="Finance Operations Hub" subtitle="Statements, classification, treasury, payments and reconciliation."/><div className="role-content">
    ...
    <section className="chart-grid"><BarChart title="Cash by Bank / Wallet" eyebrow="Treasury" items={accounts.map((a:any)=>({label:a.institution_name||a.account_name,note:a.account_name,value:Number(a.current_balance||0)}))}/><DonutCard title="Statement Resolution" eyebrow="Reconciliation workload" pct={resolvedPct} label="resolved" stats={[["Auto-posted",auto],["Already known",known],["Needs action",unclassifiedEstimate]]}/></section>
    <section className="role-grid role-grid-wide"><article className="role-card"><CardHead eyebrow="Transactions" title="Recent Confirmed Movements" action={<Link href="/statements">Open inbox</Link>}/>{transactions.slice(0,10).map((tx:any)=><DataLine key={tx.id} label={tx.narration||"Transaction"} note={`${tx.transaction_date} · ${String(tx.classification||"unclassified").replaceAll("_"," ")}`} value={money(tx.signed_amount)} warn={!tx.project_id}/>)}</article><article className="role-card opay-inspired-card"><CardHead eyebrow="Banking" title="Quick Balance" action={<Link href="/treasury">Manage accounts</Link>}/><div className="wallet-balance"><small>Total recorded balance</small><strong>{money(bankCash)}</strong><span>OPay-inspired quick view.</span></div>{accounts.slice(0,5).map((a:any)=><DataLine key={a.id} label={a.institution_name||a.account_name} note={a.account_name} value={money(a.current_balance)}/>)}</article></section>
    <LineChart title="Money In vs Money Out" eyebrow="Recent confirmed cashflow" data={monthly}/>
  </div></>;
}
...
function buildMonthlySeries(transactions:any[]){
  ...
  for(const tx of [...transactions].reverse()){
    const date=String(tx.transaction_date||""); if(date.length<7)continue; const key=date.slice(0,7);
    const [y,m]=key.split("-").map(Number); const label=new Date(y,m-1,1).toLocaleDateString("en-NG",{month:"short"});
    const row=map.get(key)||{key,label,inflow:0,outflow:0}; const amount=Number(tx.signed_amount||0); if(amount>=0)row.inflow+=amount;else row.outflow+=Math.abs(amount); map.set(key,row);
  }
...
function categoryTotals(transactions:any[]){
  ...
  for(const tx of transactions){if(Number(tx.signed_amount||0)>=0)continue;const k=String(tx.category_name||"Uncategorised");map.set(k,(map.get(k)||0)+Math.abs(Number(tx.signed_amount||0)));}
  return [...map.entries()].map(([label,value])=>({label,note:"",value})).sort((a,b)=>b.value-a.value).slice(0,6);
}

function makePath(values:number[],max:number){const w=600,h=150,p=18,n=Math.max(values.length,1),step=n>1?(w-p*2)/(n-1):0;return values.map((v,i)=>`${i?"L":"M"} ${p+i*step} ${h-p-(max?((v/max)*(h-p*2)):0)}`).join(" ")}

function LineChart({title,eyebrow,data}:{title:string;eyebrow:string;data:any[]}){
  const inflow=data.map((d:any)=>Number(d.inflow||0)),outflow=data.map((d:any)=>Number(d.outflow||0));const max=Math.max(1,...inflow,...outflow);const w=600,h=150,p=18;
  return <article className="chart-card"><div className="chart-head"><div><small>{eyebrow}</small><h3>{title}</h3></div><div className="chart-legend"><span><i className="legend-a"/>Money in</span><span><i className="legend-b"/>Money out</span></div></div>{data.length?<svg className="line-chart" viewBox={`0 0 ${w} ${h}`} role="img" aria-label={title}>{[.25,.5,.75,1].map(v=><line key={v} className="grid" x1={p} x2={w-p} y1={h-p-v*(h-p*2)} y2={h-p-v*(h-p*2)}/>)}<path className="line-a" d={makePath(inflow,max)}/><path className="line-b" d={makePath(outflow,max)}/>{data.map((d:any,i:number)=>{const x=p+(data.length>1?i*(w-p*2)/(data.length-1):0);return <text key={d.key} x={x} y={h-3} textAnchor="middle">{d.label}</text>})}</svg>:<Empty text="Confirmed transaction history will populate this chart."/>}</article>
}
...
function BarChart({title,eyebrow,items}:{title:string;eyebrow:string;items:{label:string;note?:string;value:number}[]}){
  ...
  return <article className="chart-card"><div className="chart-head"><div><small>{eyebrow}</small><h3>{title}</h3></div></div><div className="bar-list">{items.length?items.slice(0,7).map((item,i)=><div className="bar-row" key={`${item.label}-${i}`}><div><b>{item.label}</b>{item.note&&<small>{item.note}</small>}</div><div className="bar-track"><i style={{width:`${Math.max(2,Math.round(Math.abs(item.value||0)/max*100))}%`}}/></div><strong>{compactMoney(item.value)}</strong></div>):<Empty text="No data yet."/>}</div></article>
}
