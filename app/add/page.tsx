...
export default async function AddPage({searchParams}:{searchParams:Promise<{projectId?:string;onboarding?:string}>}){
  ...
  return <main className="simple-shell"><div className="simple-wrap"><div className="simple-top"><Link href={onboarding?"/onboarding/start":"/"}>← {onboarding?"Start choices":"Home"}</Link><span style={{display:"flex",gap:12}}><Link href="/documents">Documents</Link><Link href="/review">Needs decision</Link><Link href="/projects">Projects</Link></span></div>{retry&&!onboarding&&<RetryStoredUpload documentId={retry.document.id} batchId={retry.batch_id} fileName={retry.document.file_name} message={retry.message}/>}<UniversalIntakeV6 companyId={membership.company_id} projects={projects??[]} defaultProjectId={defaultProjectId} onboarding={onboarding}/></div></main>;
}
