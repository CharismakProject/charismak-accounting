...
export default async function AddPage({searchParams}:{searchParams:Promise<{projectId?:string;onboarding?:string}>}){
  ...
  return <main className="simple-shell"><div className="simple-wrap">
    <div className="simple-top"><Link href={onboarding?"/onboarding/start":"/"}>← {onboarding?"Start choices":"Home"}</Link><span style={{display:"flex",gap:12}}><Link href="/documents">Documents</Link><Link href="/review">Needs decision</Link><Link href="/projects">Projects</Link></span></div>
    {!onboarding&&<section className="compact-card" style={{marginBottom:14,display:"flex",justifyContent:"space-between",alignItems:"center",gap:14,flexWrap:"wrap"}}><div><p className="page-eyebrow green" style={{margin:0}}>NO STATEMENT REQUIRED</p><h2 style={{margin:"4px 0"}}>Enter money manually</h2><p style={{margin:0,fontSize:11,color:"#6f8092"}}>Record funding, expenses, income, loans or advances directly. Statements remain optional.</p></div><Link href="/add/manual" className="primary-link-button">Record money →</Link></section>}
    {retry&&!onboarding&&<RetryStoredUpload documentId={retry.document.id} batchId={retry.batch_id} fileName={retry.document.file_name} message={retry.message}/>}<UniversalIntakeV6 companyId={membership.company_id} projects={projects??[]} defaultProjectId={defaultProjectId} onboarding={onboarding}/>
  </div></main>;
}
