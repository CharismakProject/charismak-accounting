...
type FingerprintResult = { ok?: boolean; duplicate?: boolean; fileHash?: string; serverVerified?: boolean; cleanedUp?: boolean; existing?: { id: string; project_id?: string | null; document_type?: string | null; file_name?: string | null } | null; error?: string };
...
export default function UniversalIntakeV6({ companyId, projects, onboarding = false, defaultProjectId = "" }: { companyId: string; projects: Project[]; onboarding?: boolean; defaultProjectId?: string }) {
  ...
  function duplicateHref(existing: FingerprintResult["existing"]) {
    if (!existing) return "/documents";
    if (existing.project_id) return `/projects/${existing.project_id}/documents`;
    return existing.document_type === "bank_statement" ? "/statements" : "/documents";
  }
  ...
  async function cleanupTemporaryUpload(path: string) {
    const cleaned = await supabase.functions.invoke("fingerprint-upload", {
      body: { action: "cleanup", companyId, bucket: "universal-intake", storagePath: path }
    });
    return !cleaned.error && Boolean(cleaned.data?.cleanedUp);
  }

  async function verifyServerFingerprint(path: string, file: File, clientHash: string | null): Promise<FingerprintResult> {
    const check = await supabase.functions.invoke("fingerprint-upload", {
      body: {
        action: "fingerprint",
        companyId,
        bucket: "universal-intake",
        storagePath: path,
        fileName: file.name,
    ...
    const data = (check.data || {}) as FingerprintResult;
    if (check.error || data.error || !data.serverVerified || !data.fileHash) {
      const detail = data.error || await readableFunctionError(check.error);
      throw new Error(`Duplicate-safety verification failed. Nothing was imported. ${detail}`);
    }
    return data;
  }
  ...
  async function applyResult(i: number, documentId: string, intakeItemId: string, analysed: any, terms: string[]): Promise<"done" | "review" | "duplicate"> {
    ...
    if (ids.length) {
      ...
      const { data: imported } = await supabase.from("statement_imports")
        .select("rows_total,rows_new,rows_already_known,rows_pending_review")
        .eq("id", ids[0])
        .maybeSingle();
      const total = Number(imported?.rows_total ?? 0);
      const newRows = Number(imported?.rows_new ?? 0);
      const knownRows = Number(imported?.rows_already_known ?? 0);
      const allKnown = total > 0 && newRows === 0 && knownRows >= total;
      const base = analysed?.message || "Financial statement understood.";
      const state: "done" | "review" | "duplicate" = allKnown ? "duplicate" : analysed?.status === "needs_review" ? "review" : "done";
      const duplicateNote = allKnown ? ` All ${total} transaction row${total === 1 ? " was" : "s were"} already known; no duplicate accounting was created.` : "";
      update(i, {
        state,
        type: "Financial statement",
        message: `${base}${duplicateNote} ${s.autoPosted} high-confidence row${s.autoPosted === 1 ? "" : "s"} posted; ${s.pending} still need review.${s.candidates ? ` ${s.candidates} possible new project/site signal${s.candidates === 1 ? "" : "s"} found.` : ""}`,
    ...
    if (analysed?.projectId && analysed?.status === "ready") {
      ...
      if (!applied.error && applied.data?.status === "needs_review") {
        update(i, { state: "review", type: label(analysed?.type || hint), message: applied.data?.message || "Document understood; confirm how it should affect the project before accounting changes.", documentId, intakeItemId, href: `/review/${intakeItemId}` });
        return "review";
      }
    }
  ...
  async function processFiles(files: File[]) {
    ...
    try {
      ...
      for (const file of files) {
        ...
        if (!isSupportedIntakeExtension(ext)) throw new Error(`${file.name}: use PDF/scanned PDF, Excel XLSX/XLS, Word DOCX, or JPG/JPEG.`);
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        const file = files[i];
        const ext = extensionFromFileName(file.name);
        const route = intakeRouteForExtension(ext);
        let documentId: string | undefined;
        let intakeItemId: string | undefined;
        let storagePath: string | undefined;

        try {
          update(i, { state: "working", message: "Checking duplicate safety…" });
          const clientHash = await optionalFileHash(file);
          if (clientHash) {
            const { data: existing } = await supabase.from("source_documents")
              .select("id,project_id,document_type,file_name")
              .eq("company_id", companyId)
              .eq("file_hash", clientHash)
              .limit(1)
              .maybeSingle();
            if (existing) {
              duplicate++;
              update(i, { state: "duplicate", message: `Already uploaded as ${existing.file_name || "an existing record"}. Nothing was counted twice.`, documentId: existing.id, href: duplicateHref(existing) });
              continue;
            }
          }

          const path = `${companyId}/intake/${new Date().getUTCFullYear()}/${Date.now()}-${crypto.randomUUID().slice(0, 8)}-${safeUploadFileName(file.name)}`;
          storagePath = path;
          update(i, { message: "Uploading the original file securely…" });
          await storeOriginal(path, file);

          update(i, { message: "Verifying the file on the server before analysis…" });
          const fingerprint = await verifyServerFingerprint(path, file, clientHash);
          const fileHash = String(fingerprint.fileHash);
          if (fingerprint.duplicate && fingerprint.existing) {
            if (!fingerprint.cleanedUp) await cleanupTemporaryUpload(path);
            storagePath = undefined;
            duplicate++;
            update(i, { state: "duplicate", message: `Already uploaded as ${fingerprint.existing.file_name || "an existing record"}. Server verification stopped the duplicate before accounting analysis.`, documentId: fingerprint.existing.id, href: duplicateHref(fingerprint.existing) });
            continue;
          }
          ...
          const doc = await supabase.from("source_documents").insert({
            ...
            metadata: {
              ...
              intake_project_hint: projectId || null,
              intake_document_type_hint: hint,
              intake_action: action,
              intake_keywords: terms,
              android_direct_upload: true,
              server_fingerprint_verified: true,
              intake_route: route,
            },
            uploaded_by: user.id
          }).select("id").single();
          ...
          if (doc.error || !doc.data) {
            if (fileHash && isDuplicateConstraintError(doc.error)) {
              const { data: existing } = await supabase.from("source_documents")
                .select("id,project_id,document_type,file_name")
                .eq("company_id", companyId)
                ...
                .limit(1)
                .maybeSingle();
              await cleanupTemporaryUpload(path);
              storagePath = undefined;
              if (existing) {
                duplicate++;
                update(i, { state: "duplicate", message: `Already uploaded as ${existing.file_name || "an existing record"}. Database protection blocked the duplicate.`, documentId: existing.id, href: duplicateHref(existing) });
                continue;
              }
            }
            await cleanupTemporaryUpload(path);
          ...
          if (action === "store_only") {
            ...
            update(i, { state: "done", type: label(selectedType), message: "Stored safely after duplicate verification. No accounting figures were changed.", documentId: currentDocumentId, intakeItemId: currentIntakeItemId, href: projectId ? `/projects/${projectId}/documents` : "/documents" });
            continue;
          }
          ...
          if (route === "image") {
            ...
            update(i, { message: route === "spreadsheet" ? "Reading the Excel workbook…" : route === "word" ? "Reading the Word document…" : "Reading and understanding the PDF…" });
            ...
            if (route === "pdf" && (normal.error || normal.data?.status === "needs_review") && needsOcrFallback(normalMessage)) {
              analysed = await ocrAnalyse(i, file, currentDocumentId, batch.id, currentIntakeItemId, terms);
            } else {
          ...
          const outcome = await applyResult(i, currentDocumentId, currentIntakeItemId, analysed, terms);
          ...
          if (intakeItemId) {
            ...
            failed++;
            if (storagePath) await cleanupTemporaryUpload(storagePath);
      ...
      setSummary(`${done} organised automatically · ${review} need a decision · ${duplicate} duplicate/already-known · ${failed} failed.`);
      router.refresh();
    } catch (e: any) {
  ...
  return <div className="universal-add">
    <section className="add-hero">
      ...
      <h1>Upload it. Charismak checks, reads and organises it.</h1>
      <p>Choose an Excel file, Word document, PDF, scanned PDF or JPG. Duplicate protection runs before accounting analysis, then Charismak connects reliable information to your company and projects.</p>
    </section>

    <section className="add-card">
      <div style={{ padding: 12, borderRadius: 12, background: "#f2f8fb", fontSize: 11, lineHeight: 1.55, color: "#456276", marginBottom: 12 }}>
        <b style={{ color: "#153c57" }}>Recommended: leave everything on automatic.</b> If Charismak is unsure, it sends the item to Needs decision instead of guessing. If duplicate verification cannot be completed, the file is not imported.
      </div>
      ...
      <label className="add-drop">
        <input type="file" multiple disabled={busy} accept={INTAKE_ACCEPT} onChange={e => { const input = e.currentTarget; const files = Array.from(input.files || []); input.value = ""; void processFiles(files); }} />
        <strong>{busy ? "Checking, uploading and understanding your records…" : "Choose files or take a JPG photo"}</strong>
        <span>{busy ? "Please keep this page open while the files are processed." : `Upload starts immediately · ${INTAKE_FORMAT_HELP}`}</span>
      </label>
