...
function loadScript(src:string,globalName:string){
  ...
  const existing=document.querySelector(`script[data-charismak-src="${src}"]`) as HTMLScriptElement|null;
...
export async function readVisualDocument(file:File,progress:Progress=()=>{}):Promise<{text:string;confidence:number;pages:number}> {
  ...
  try{
    ...
    for(let i=1;i<=document.numPages;i++){
      progress(`Reading scanned PDF page ${i} of ${document.numPages}…`);
      const page=await document.getPage(i);const viewport=page.getViewport({scale:1.45});
      const canvas=document.createElement("canvas");const context=canvas.getContext("2d",{willReadFrequently:true});if(!context)throw new Error("Your browser could not prepare this scanned page for reading.");
